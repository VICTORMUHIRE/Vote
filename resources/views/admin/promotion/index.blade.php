        ...
        <div class="container1">
            <div class="containter-child">
                ...
                <div class="add d-flex justify-content-between align-items-center mt-2">
                    <h4>Tous les promotions</h4>
                    <a class="btn btn-primary d-flex align-items-center gap-2" href="{{route("admin.promotion.create")}}">
                        ...
                        <div @style("color:white")>Nouvelle promotion</div>
                    </a>
                </div>
                <table class="table1">
                    <thead class="thead1">
                    ...
                    <tbody class="tbody1">
                        @foreach ($promotions as $prom)
                            <tr>
                                <td class="d-flex justify-content-start"  >{{$prom->name}}</td>
                                <td>{{$prom->alias}}</td>
                                <td>{{$prom->faculte->alias}}</td>
                                <td>
                                    <div class="d-flex gap-2 w-100 justify-content-end">
                                        <a onclick="popUP()" href="{{route('admin.promotion.edit',$prom)}}" class="btn btn-primary">Editer</a>
                                        <form method="POST" action="{{route('admin.promotion.destroy', $prom)}}">
                                            @csrf
                                            @method('delete')
